...
function getTotalUsersPages () {
    $getAll = "SELECT COUNT(*) FROM ".$this-> table_name ." WHERE rights != 3";
...
function getAllUsers () {
    $getAllUsers = "SELECT id, organization, firstname, lastname, email, phone, rights, updated, created FROM ".$this-> table_name ." WHERE rights != 3 LIMIT ".$this->fromPage.", ".$this->pageNumber."";
        ...
        echo json_encode(array("error" => "пользователей не существует"));
        exit();
        }
}
...
function approveUser() {
    $query = "UPDATE ".$this-> table_name." SET rights = 1, updated = CURRENT_TIMESTAMP WHERE id = :id AND email = :email";
        $stmt = $this->conn -> prepare($query);
        ...
        http_response_code(400);
        echo json_encode(array("error" => "can't_approve_user"));
        exit();
}

function rejectUser() {
    $query = "UPDATE ".$this-> table_name." SET rights = 2, updated = CURRENT_TIMESTAMP WHERE id = :id AND email = :email";
        $stmt = $this->conn -> prepare($query);
        $this-> id = htmlspecialchars(strip_tags($this-> id));
        $this-> email = htmlspecialchars(strip_tags($this-> email));
        ...
        http_response_code(400);
        echo json_encode(array("error" => "can't_reject_user"));
        exit();
}

function deleteUser() {
    $query = "UPDATE ".$this-> table_name." SET rights = 3, updated = CURRENT_TIMESTAMP WHERE id = :id AND email = :email";
        $stmt = $this->conn -> prepare($query);
        $this-> id = htmlspecialchars(strip_tags($this-> id));
        $this-> email = htmlspecialchars(strip_tags($this-> email));
        ...
        http_response_code(400);
        echo json_encode(array("error" => "can't_delete_user"));
        exit();
}
